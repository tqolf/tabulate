...
template <size_t Capacity, size_t BitsetWidth = 16, typename BitsetWord = unsigned long, size_t MaxTries = 32,
...
class atomic_bitset {
            ...
            if (ATOMIC_BITSET_UNLIKELY(elt == nullptr)) {
                ...
                if (elements[bucket].compare_exchange_weak(elt, newelt, std::memory_order_release,
                    ...
                    if (elt->cardinality == cardinality) {
                        ...
                        tries++;
                        relocate(bucket, bucket, index, offset);
                    }
                }
    ...
    status reset(Index pos)
    {
        ...
        while (tries < MaxTries) {
            ...
            if (ATOMIC_BITSET_UNLIKELY(val == nullptr)) {
                ...
                val->words[index].fetch_and(~(1 << offset));
                return status::success;
            } else {
                tries++;
                relocate(bucket, bucket, index, offset);
    ...
    status test(Index pos) const
    {
        ...
        while (tries < MaxTries) {
            ...
            if (ATOMIC_BITSET_UNLIKELY(val == nullptr)) {
                ...
                return (val->words[index].load() & (1 << offset)) ? status::yes : status::no;
            } else {
                tries++;
                relocate(bucket, bucket, index, offset);
            }
  ...
  private:
    struct BitSetElement {
        ...
        {
            for (size_t i = 0; i < (1 << BitsetWidth) / (8 * sizeof(BitsetWord)); i++) {
                words[i].store(0, std::memory_order_seq_cst);
            }
            if (offset <= 8 * sizeof(BitsetWord) - 1) {
                words[index].fetch_or(1 << offset, std::memory_order_seq_cst);
            }
        }
        ...
        {
            words[index].fetch_and(~(1 << offset), std::memory_order_seq_cst);
        }
    };
