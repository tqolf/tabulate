...
AEAD_WITH_EVP::AEAD_WITH_EVP(const EVP_CIPHER *method) : m_method(method), m_context(nullptr) {}
...
int AEAD_WITH_EVP::reset(int mode, const std::string &key, const std::string &iv)
{
    if (key.empty() || static_cast<size_t>(EVP_CIPHER_key_length(m_method)) != key.size()) {
        RESUMABLECACHE_TRACE("invalid key size(key.size() = %zu)", key.size());
        return EINVAL;
    }
    if (mode == AUTO) {
    ...
    if (m_context == nullptr) {
        ...
        if (!EVP_CipherInit_ex(m_context, m_method, NULL, NULL, NULL, mode == ENCRYPT)) {
            RESUMABLECACHE_TRACE("Failed to initialise the cryption operation");
    ...
    if (iv.size() != 0 && iv.size() != 12) {
        if (!EVP_CIPHER_CTX_ctrl(m_context, EVP_CTRL_GCM_SET_IVLEN, iv.size(), NULL)) {
            RESUMABLECACHE_TRACE("Failed to set IV length");
            return static_cast<int>(ERR_get_error());
        }
    }
    ...
    if (!EVP_CipherInit_ex(m_context, NULL, NULL, (const unsigned char *)key.data(), (const unsigned char *)iv.data(),
                           mode == ENCRYPT)) {
        RESUMABLECACHE_TRACE("Failed to initialise key and IV");
        return static_cast<int>(ERR_get_error());
    }
...
int AEAD_WITH_EVP::crypt(const std::string &aad, const std::string &input, std::string &output)
{
    ...
    if (EVP_CIPHER_CTX_encrypting(m_context)) {
        outsz = input.size() + EVP_GCM_TLS_TAG_LEN;
    } else {
        if (input.size() < EVP_GCM_TLS_TAG_LEN) {
            return -EINVAL;
        }
        outsz = input.size() - EVP_GCM_TLS_TAG_LEN;
        if (outsz == 0) {
    ...
    int outl = 0;
    if (!EVP_CipherUpdate(m_context, NULL, &outl, (const unsigned char *)aad.data(), aad.size())) {
        RESUMABLECACHE_TRACE("Failed to intialize additional application data.");
        ERR_print_errors_fp(stderr);
        return static_cast<int>(ERR_get_error());
    }
    size_t inlen = input.size() - (!EVP_CIPHER_CTX_encrypting(m_context) ? EVP_GCM_TLS_TAG_LEN : 0);
    ...
    if (!EVP_CipherUpdate(m_context, out, &outl, (const unsigned char *)input.data(), inlen)) {
        RESUMABLECACHE_TRACE("Failed to obtain the crypted output.");
        return static_cast<int>(ERR_get_error());
    }
    assert(outsz >= outl);
    out += outl;
    ...
    if (!EVP_CIPHER_CTX_encrypting(m_context)) {
        /* Set the tag */
        if (!EVP_CIPHER_CTX_ctrl(m_context, EVP_CTRL_GCM_SET_TAG, EVP_GCM_TLS_TAG_LEN,
                                 (void *)((unsigned char *)input.data() + inlen))) {
            RESUMABLECACHE_TRACE("Failed to get the tag");
            return static_cast<int>(ERR_get_error());
        }
    }
    ...
    if (!EVP_CipherFinal_ex(m_context, out, &outl)) {
        RESUMABLECACHE_TRACE("Failed to finalise the cryption");
        return static_cast<int>(ERR_get_error());
    }
    assert(outsz >= outl);
    out += outl;
    ...
    if (EVP_CIPHER_CTX_encrypting(m_context)) {
        /* Get the tag */
        if (!EVP_CIPHER_CTX_ctrl(m_context, EVP_CTRL_GCM_GET_TAG, EVP_GCM_TLS_TAG_LEN, out)) {
            RESUMABLECACHE_TRACE("Failed to get the tag");
            return static_cast<int>(ERR_get_error());
        }
        assert(outsz >= EVP_GCM_TLS_TAG_LEN);
        out += EVP_GCM_TLS_TAG_LEN;
        outsz -= EVP_GCM_TLS_TAG_LEN;
    }
