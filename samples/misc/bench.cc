...
int main(int argc, char **argv)
{
    ...
    enum {
        SELECT_TIMES = 0x8001,
        SELECT_SAMPLING = 0x8002,
        SELECT_DATA_STRUCTURE = 0x8004,
        SELECT_AEAD = 0x08,
    };
    unsigned selections = SELECT_AEAD;

    profiler::SetTitle("Benchmarks");

    if (selections & 0x8000) {
    ...
    if (selections & SELECT_DATA_STRUCTURE) {
        struct range {
            size_t min;
            size_t max;
    ...
    if (selections & SELECT_AEAD) {
        std::string iv = RandomBytes(16);
        std::string key = RandomBytes(16);
        std::string plaintext = RandomBytes(256);

        std::cout << "IV: " << ToHex(iv) << std::endl;
        std::cout << "Key: " << ToHex(key) << std::endl;
        std::cout << "Plaintext: " << ToHex(plaintext) << std::endl << std::endl;

        AEAD_WITH_EVP encontext, decontext;
        if (int err = encontext.reset(AEAD_WITH_EVP::ENCRYPT, key, iv); err != 0) {
            return err;
        }
        if (int err = decontext.reset(AEAD_WITH_EVP::DECRYPT, key, iv); err != 0) {
            return err;
        }
        ...
        auto aead_aes_once = [&](const std::string &key [[maybe_unused]], const std::string &iv [[maybe_unused]],
            ...
            // encontext.reset(AEAD_WITH_EVP::ENCRYPT, key, iv);
            encontext.crypt(aad, plaintext, encrypted);

            std::string decrypted;
            // decontext.reset(AEAD_WITH_EVP::DECRYPT, key, iv);
            decontext.crypt(aad, encrypted, decrypted);
        ...
        profiler::Add(
            "aead::aes",
            [&]() {
                aead_aes_once(key, iv, plaintext);
        ...
        profiler::Add(
            "ossl::aes",
            [&]() {
                ossl_aes_once(key, iv, plaintext);
